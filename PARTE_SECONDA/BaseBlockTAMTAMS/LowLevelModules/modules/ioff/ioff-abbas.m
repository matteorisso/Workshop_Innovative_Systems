...
%    Approximation of Lambert W for very little values of the argument
     ...
     Vx = [VS, VDS];
     tmp = (exp((Vod - Vx)./(2*VT)))./(1 + A*exp((Vx - Vod)./(2*eta1TG*VT)));
     i = find(tmp <= 0.001);
     qi(i) = LambertW3(tmp(i));
     i = find(tmp > 0.001);
     qi(i) = LambertW2(tmp(i));
...
CLM = 1; % channel length modulation flag
...
MoD = 1; % mobility degradation flag
...
u = u0/(1 + MoD*theta1*VT*qiS);

%% Corner effects (trapezoidal geometry)

phi = atand((Wfinbot - Wfintop)/(2*Hfin));
C = 1/(1 - 0.0084*phi*Hfin/Wfinbot) - 1;
fce = 1/(1 + C/(1 + exp(-Vod/0.05))); 

%% OFF current computation

Ioff = fce*u*Cox*Weff*((2*VT)^2)*((qiS - qiD)/L + 0.5*(qiS^2 - qiD^2)/(L - CLM*dL));

Ioff_normalized = 1e8*Ioff/Weff % [pA / um]
