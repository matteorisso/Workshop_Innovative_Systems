...
% Transistor dimensions	um
	...
	Wn2 			= 2*Wgate_n; %nm %abbiamo tolto 1e-3        		
...
% D/S dimensions
    ...
    perim_N = 2*lungh_diff*1e3 + Wn2;     %nm = [um]*1000+[nm]
...
% Capacitances
	...
	C_ox_n 		= 	Cox.*Wn2.*Leff.*1e-6;%pF effective nMOS oxide capacitance 
	C_ox_p 		= 	Cox.*Wp.*Leff.*1e-6;%pF effective pMOS oxide capacitance

% MOS resistances for delay

	Rn 			= 	1/(mueff_n*Cox*(Wn2/Leff)*(Vdd-Vth_n))*1e12;	%Ohm nMOS channel resistance
	Rp			= 	1/(mueff_p*Cox*( Wp/Leff)*abs(Vth_p-Vdd))*1e12;	%Ohm pMOS channel resistance

% Ig

	Igate_nmos 	= 	Igate_n.*Wn2/1000;    %nA=[nA/um]*(nm/1000);
	Igate_pmos 	= 	Igate_p.*Wp/1000;     %nA=[nA/um]*(nm/1000);

% Ioff

	Ioff_nmos 	= 	Ioff_n.*Wn2/1000;          %nA=[nA/um]*(nm/1000);
...
% Dynamic power
	...
	Cjn			=	(Cbottom_n.*Wn2+Csidewall_n.*perim_N)*1e-3;		%pF
	Cjp			=	(Cbottom_p.*Wp +Csidewall_p.*perim_P)*1e-3;	    %pF
	CinN 		= 	Cox*Leff*1e-6*Wn2 + 2*Coverlap_n ;	%pF
    ...
    Cx          =   2*Cjn; %since Cx1=Cx2. 
    
    figure;
	for h=1:4 % Different fanouts
		i(h) = h;
        Cy = Cjp + Cjn + h*Cl;
		Calpha = 3*CinN + 1.75*Cx + 0.25*Cy +  2*CinP;
		Power_dynamic_nor3(h) = 0.5*freq*Calpha*Vdd^2*1e3; % microW	
        hold on
        plot(h,Power_dynamic_nor3(h),'*')
        title('Dynamic power NOR3 computed with different fanout values');
        xlabel('Fanout');
        ylabel('Dynamic Power [\muW]');
        grid on
    end ;
    p = polyfit(i,Power_dynamic_nor3,1);
    x1 = linspace(0,4,41);
    y1 = polyval(p,x1);
    plot(x1,y1);
...
% Delay
	...
	for h=1:4 % Different fanouts
		...
		CMOS_tpdf = Rn*(5*Cjn + 2*Cjp + 2*h*Cl);
	    t_delay(h)= max(CMOS_tpdr,CMOS_tpdf)/1e3; % ps
        hold on
        plot(h,t_delay(h),'*')
        title('Max rise/fall time delay NOR3 computed with different fanout values');
        xlabel('Fanout');
...
% Static power
	...
	I_leak = (1/16)*( 8*Igate_pmos + 7*Ioff_pmos + 40*Igate_nmos + 54*Ioff_nmos); % nA
	Power_static_nor3 = Vdd*I_leak %nW