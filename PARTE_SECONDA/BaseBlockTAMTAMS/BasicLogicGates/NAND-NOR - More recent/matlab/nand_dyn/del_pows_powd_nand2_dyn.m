...
% Transistor dimensions	um
	...
	Wn3 			= 3*Wgate_n; %nm %abbiamo tolto 1e-3        		
...
% D/S dimensions
    ...
    perim_N = 2*lungh_diff*1e3 + Wn3;     %nm = [um]*1000+[nm]
...
% Capacitances
	...
	C_ox_n 		= 	Cox.*Wn3.*Leff.*1e-6;%pF effective nMOS oxide capacitance 
	C_ox_p 		= 	Cox.*Wp.*Leff.*1e-6;%pF effective pMOS oxide capacitance

% MOS resistances for delay

	Rn 			= 	1/(mueff_n*Cox*(Wn3/Leff)*(Vdd-Vth_n))*1e12;	%Ohm nMOS channel resistance
	Rp			= 	1/(mueff_p*Cox*( Wp/Leff)*abs(Vth_p-Vdd))*1e12;	%Ohm pMOS channel resistance

% Ig

	Igate_nmos 	= 	Igate_n.*Wn3/1000;    %nA=[nA/um]*(nm/1000);
	Igate_pmos 	= 	Igate_p.*Wp/1000;     %nA=[nA/um]*(nm/1000);

% Ioff

	Ioff_nmos 	= 	Ioff_n.*Wn3/1000;          %nA=[nA/um]*(nm/1000);
...
% Dynamic power
	...
	Cin 		= 	C_ox_n+C_ox_p+2*Coverlap_n+2*Coverlap_p;  %pF
	Cl 			=  	C_ox_n+C_ox_p+2*Coverlap_n+2*Coverlap_p;        %pF
	Cjn			=	(Cbottom_n.*Wn3+Csidewall_n.*perim_N)*1e-3;		%pF
	Cjp			=	(Cbottom_p.*Wp +Csidewall_p.*perim_P)*1e-3;	    %pF
	CinN 		= 	Cox*Leff*1e-6*Wn3 + 2*Coverlap_n ;	%pF
	...
	for h=1:4 % Different fanouts
        i(h) = h;
		Cy = Cjp + Cjn + h*Cl;
		Calpha = 3*CinN + 3*Cx + 1.5*Cy + 2*CinP;  %pF
		Power_dynamic_nd2(h) = 0.5*freq*Calpha*Vdd^2*1e3; % microW	
        hold on
        plot(h,Power_dynamic_nd2(h),'*')
        title('Dynamic power NAND2 computed with different fanout values');
        xlabel('Fanout');
        ylabel('Dynamic Power [\muW]');
        grid on
    end;
    p = polyfit(i,Power_dynamic_nd2,1);
    x1 = linspace(0,4,41);
    y1 = polyval(p,x1);
    plot(x1,y1);
...
% Delay
	...
	for h=1:4 % Different fanouts
        i(h) = h;
		CMOS_tpdr = Rp*(Cjp + Cjn + h*Cl);         %ps
		CMOS_tpdf = Rn*(6*Cjn + 3*Cjp + 3*h*Cl);   %ps
		t_delay(h)= max(CMOS_tpdr,CMOS_tpdf)/1e3;  %ns
        hold on
        plot(h,t_delay(h),'*')
        title('Max rise/fall time delay NAND2 computed with different fanout values');
        xlabel('Fanout');
...
% Static power
	...
	I_leak = (1/8)*( 4*Igate_pmos + Ioff_pmos + 21*Igate_nmos + 21*Ioff_nmos); % nA
	Power_static_nand2 = Vdd*I_leak %nW
