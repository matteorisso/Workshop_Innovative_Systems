classdef nand2
    ...
    methods
        ...
        function obj = nand2(nMOS_width)
            ...
            [obj.I_nMOSHP, obj.I_pMOSHP, obj.e0]= obj.Ion_Mastar_modelHP2010(obj.Vth_nHP); % [uA/um]
            obj.C_nd2_HP = 13.74 * (obj.eps_SiO2*obj.L_HP2010/obj.Tox_HP2010 + obj.fring_capHP2010 ...
            ...
            [obj.I_nMOSLOP, obj.I_pMOSLOP]= obj.Ion_Mastar_modelLOP2010(obj.Vth_nLOP); % [uA/um]
            obj.C_nd2_LOP = 13.74 * (obj.eps_SiO2*obj.L_LOP2010/obj.Tox_LOP2010 + obj.fring_capLOP2010 ...
            ...
            [obj.I_nMOSLSTP, obj.I_pMOSLSTP]= obj.Ion_Mastar_modelLSTP2010(obj.Vth_nLSTP); % [uA/um]
            obj.C_nd2_LSTP = 13.74 * (obj.eps_SiO2 * obj.L_LSTP2010 / obj.Tox_LSTP2010 + obj.fring_capLSTP2010 ...
        ...
        function [Tdp_HP_nd2, Tdp_LOP_nd2, Tdp_LSTP_nd2] = delay(obj)
            % HP
            Tdp_HP_nd2 =  obj.C_nd2_HP * obj.VDD_HP2010 / (obj.I_pMOSHP * obj.Wp * 1e-3 * 1e-6); % [s]
            % LOP
            Tdp_LOP_nd2 = obj.C_nd2_LOP * obj.VDD_LOP2010 /(obj.I_pMOSLOP * obj.Wp * 1e-3 * 1e-6); % [s]
            % LSTP
            Tdp_LSTP_nd2 = obj.C_nd2_LSTP * obj.VDD_LSTP2010 / (obj.I_pMOSLSTP * obj.Wp * 1e-3 * 1e-6); % [s]
        end
        
        % Area evaluation
        function [A_HP_nd2, A_LOP_nd2, A_LSTP_nd2] = area(obj)
           % HP
           A_HP_nd2 = 2 * (obj.Wn + obj.Wp) * (obj.L_HP2010 * (1 + 2 * obj.lambda)) ...
               * (1 + obj.interc_override) * 1e-6; % [um^2]
           % LOP
           A_LOP_nd2 = 2 * (obj.Wn + obj.Wp) * (obj.L_LOP2010 * (1 + 2 * obj.lambda)) ...
               * (1 + obj.interc_override) * 1e-6; % [um^2]
           % LSTP
           A_LSTP_nd2 = 2 * (obj.Wn + obj.Wp) * (obj.L_LSTP2010 * (1 + 2 * obj.lambda)) ...
               * (1 + obj.interc_override) * 1e-6; % [um^2]
        end
        ...
        function [Pdyn_HP_nd2, Pdyn_LOP_nd2, Pdyn_LSTP_nd2] = power_dyn(obj)
            % HP
            Pdyn_HP_nd2 = obj.Pdyn_HP_FO4; % [W]
            % LOP
            Pdyn_LOP_nd2 = obj.Pdyn_LOP_FO4; % [W]
            % LSTP
            Pdyn_LSTP_nd2 = obj.Pdyn_LSTP_FO4; % [W]
        end
        
        % Static power evaluation
        function [Pstat_HP_nd2, Pstat_LOP_nd2, Pstat_LSTP_nd2] = power_stat(obj)
            % HP
            Pstat_HP_nd2 = obj.Pstat_HP_FO4; % [W]
            % LOP
            Pstat_LOP_nd2 = obj.Pstat_LOP_FO4; % [W]
            % LSTP
            Pstat_LSTP_nd2 = obj.Pstat_LSTP_FO4; % [W]
        end
