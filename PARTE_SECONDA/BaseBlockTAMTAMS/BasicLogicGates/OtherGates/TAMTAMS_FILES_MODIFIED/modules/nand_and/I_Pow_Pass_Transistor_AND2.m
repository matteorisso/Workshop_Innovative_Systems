%% PASS-TRANSISTOR AND 2 POWER ANALYSIS
% Implementation of a model for the evaluation of power consumption of 2 inputs PASS-TRANSISTOR NAND-AND gates
%%
...
% Module name: I_Pow_Pass_Transistor_AND2.m
% Stored informations:  Iand2_pass_btb, Iand2_pass_gate, Iand2_pass_off, Pand2, Pand2_dyn
...
Ibtb_n_pass2=Ibtb_nmos.*Wn;      % nA
Ibtb_p_pass2=Ibtb_pmos.*Wp;      % nA

I_0_0=Ibtb_n_pass2+4.*Ibtb_p_pass2;	%nA
I_0_1=5.*Ibtb_p_pass2;			%nA
I_1_0=2.*Ibtb_n_pass2+3.*Ibtb_n_pass2;	%nA
I_1_1=2.*Ibtb_p_pass2+3.*Ibtb_n_pass2;	%nA

Iand2_pass_btb(counter_mod_index, counter_tech_index, i)=(I_0_0+I_0_1+I_1_0+I_1_1)./4;	% nA

Ibtb_nand_and(counter_mod_index, counter_tech_index,i)=Iand2_pass_btb(counter_mod_index, counter_tech_index,i);
...
for j=1:length(MULTIVDD)
		Pand2_btb(counter_mod_index,counter_tech_index,i)=Pand2_btb(counter_mod_index,counter_tech_index,i)+Iand2_pass_btb(counter_mod_index,counter_tech_index,i).*MULTIVDD(j).*1e-9; % W
...
Igate_n_pass2=Igate_nmos.*Wn;    % nA
Igate_p_pass2=Igate_pmos.*Wp;    % nA

I_0_0=2.*Igate_p_pass2+Igate_n_pass2;	%nA
I_0_1=2.*Igate_n_pass2+Igate_p_pass2;	%nA
I_1_0=Igate_p_pass2+Igate_n_pass2;	%nA
I_1_1=Igate_n_pass2+Igate_p_pass2;	%nA

Iand2_pass_gate(counter_mod_index, counter_tech_index, i)=(I_0_0+I_0_1+I_1_0+I_1_1)./4;	% nA 

Igate_nand_and(counter_mod_index, counter_tech_index,i)=Iand2_pass_gate(counter_mod_index, counter_tech_index,i) ;
...
for j=1:length(MULTIVDD)
		Pand2_gate(counter_mod_index,counter_tech_index,i)=Pand2_gate(counter_mod_index,counter_tech_index,i)+Iand2_pass_gate(counter_mod_index,counter_tech_index,i).*MULTIVDD(j).*1e-9; % W
...
Ioff_n_pass2=Ioff_nmos.*Wn;      % nA
Ioff_p_pass2=Ioff_pmos.*Wp;      % nA

I_0_0=Ioff_n_pass2;			%nA
I_0_1=Ioff_p_pass2;			%nA
I_1_0=Ioff_n_pass2+Ioff_p_pass2;	%nA
I_1_1=Ioff_p_pass2+Ioff_n_pass2;	%nA

Iand2_pass_off(counter_mod_index, counter_tech_index, i)=(I_0_0+I_0_1+I_1_0+I_1_1)./4;	% nA

Ioff_nand_and(counter_mod_index, counter_tech_index,i)=Iand2_pass_off(counter_mod_index, counter_tech_index,i);
...
for j=1:length(MULTIVDD)
		Pand2_off(counter_mod_index,counter_tech_index,i)=Pand2_off(counter_mod_index,counter_tech_index,i)+Iand2_pass_off(counter_mod_index,counter_tech_index,i).*MULTIVDD(j).*1e-9; % W 

endfor

endfor

% POWER STATIC Pass Transistor AND2 
...
% POWER DYNAMIC Pass Transistor AND2 

for i=1:4

	Pand2_dyn(counter_mod_index,counter_tech_index,i)=0;
	for j=1:length(MULTIVDD)
			Pand2_dyn(counter_mod_index,counter_tech_index,i)=Pand2_dyn(counter_mod_index,counter_tech_index,i)+(Cand2_pass(counter_mod_index,counter_tech_index,i).*1e-12).*Vdd_d(j).*MULTIVDD(j).^2.*freq(counter_mod_index,counter_tech_index).*1e6; % W 
	endfor
