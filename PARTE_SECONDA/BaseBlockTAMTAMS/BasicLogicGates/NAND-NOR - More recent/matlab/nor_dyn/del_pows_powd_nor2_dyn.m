...
% Dynamic power
	...
	for h=1:4 % Different fanouts
		i(h) = h;
        Cy = Cjp + Cjn + h*Cl;
		Calpha = 3*CinN + 1.5*Cx + 0.5*Cy +  2*CinP;
		Power_dynamic_nor2(h) = 0.5*freq*Calpha*Vdd^2*1e3; % microW	
        hold on
        plot(h,Power_dynamic_nor2(h),'*')
        title('Dynamic power NOR2 computed with different fanout values');
        xlabel('Fanout');
        ylabel('Dynamic Power [\muW]');
        grid on
    end ;
    p = polyfit(i,Power_dynamic_nor2,1);
    x1 = linspace(0,4,41);
    y1 = polyval(p,x1);
    plot(x1,y1);
...
% Delay
	...
	for h=1:4 % Different fanouts
		...
		CMOS_tpdf = Rn*(4*Cjn + 2*Cjp + 2*h*Cl);
	    t_delay(h)= max(CMOS_tpdr,CMOS_tpdf)/1e3; % ps
        hold on
        plot(h,t_delay(h),'*')
        title('Max rise/fall time delay NOR2 computed with different fanout values');
        xlabel('Fanout');
...
% Static power
	...
	I_leak = (1/8)*( 4*Igate_pmos + 3*Ioff_pmos + 16*Igate_nmos + 20*Ioff_nmos); % nA
	Power_static_nor2 = Vdd*I_leak %nW