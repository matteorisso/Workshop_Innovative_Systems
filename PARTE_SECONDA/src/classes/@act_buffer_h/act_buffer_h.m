classdef act_buffer_h < nand2
    % act_buffer_h class describes a sequntial circuit
    % modeled with 160 mux 8to1, 8 mux 2to1, 8 registers
    ...
    methods
        % Constructor
        function obj = act_buffer_h(nMOS_width, n_bit, array_size, kernel_size)
            % Constructor of father class nand2
            obj            = obj@nand2(nMOS_width);
            obj.N            = n_bit;
            obj.K            = kernel_size;				
            obj.W            = array_size;				
            obj.mux2to1        = muxnto1_nbit(nMOS_width, 2, 2*n_bit);
            obj.mux8to1        = muxnto1_nbit(nMOS_width, array_size, 1);
			obj.reg            = register(nMOS_width, 2*n_bit);
            obj.decoder3to8    = decoder(nMOS_width, 3);			
        end
        
        % Delay evaluation
        function [Tdp_HP_act_buffer_h, Tdp_LOP_act_buffer_h, Tdp_LSTP_act_buffer_h] = delay(obj)
            ...
            % HP
            Tdp_HP_act_buffer_h     = Tdp_HP_decoder3to8 + Tdp_HP_mux2to1 + Tdp_HP_mux8to1; % [s]
            % LOP
            Tdp_LOP_act_buffer_h     = Tdp_LOP_decoder3to8 + Tdp_LOP_mux2to1 + Tdp_LOP_mux8to1; % [s]
            % LSTP
            Tdp_LSTP_act_buffer_h     = Tdp_LSTP_decoder3to8 + Tdp_LSTP_mux2to1 + Tdp_LSTP_mux8to1; % [s]
		end
		
		% Area evaluation
        function [A_HP_act_buffer_h, A_LOP_act_buffer_h, A_LSTP_act_buffer_h] = area(obj)
            ...
            % HP
            A_HP_act_buffer_h      = (8 * A_HP_mux2to1) + A_HP_decoder3to8 + (160 * A_HP_mux8to1) + (8 * A_HP_reg); % [um^2]
            % LOP
            A_LOP_act_buffer_h      = (8 * A_LOP_mux2to1) + A_LOP_decoder3to8 + (160 * A_LOP_mux8to1) + (8 * A_LOP_reg); % [um^2]
            % LSTP
            A_LSTP_act_buffer_h      = (8 * A_LSTP_mux2to1) + A_LSTP_decoder3to8 + (160 * A_LSTP_mux8to1) + (8 * A_LSTP_reg); % [um^2]
		end
        
		
        % Dynamic power evaluation
        function [Pdyn_HP_act_buffer_h, Pdyn_LOP_act_buffer_h, Pdyn_LSTP_act_buffer_h] = power_dyn(obj)
            ...
            % HP
            Pdyn_HP_act_buffer_h      = (8 * Pdyn_HP_mux2to1) + Pdyn_HP_decoder3to8 + (160 * Pdyn_HP_mux8to1) + (8 * Pdyn_HP_reg); % [W]
            % LOP
            Pdyn_LOP_act_buffer_h      = (8 * Pdyn_LOP_mux2to1) + Pdyn_LOP_decoder3to8 + (160 * Pdyn_LOP_mux8to1) + (8 * Pdyn_LOP_reg); % [W]
            % LSTP
            Pdyn_LSTP_act_buffer_h      = (8 * Pdyn_LSTP_mux2to1) + Pdyn_LSTP_decoder3to8 + (160 * Pdyn_LSTP_mux8to1) + (8 * Pdyn_LSTP_reg); % [W]
		end
        
        % Static power evaluation
        function [Pstat_HP_act_buffer_h, Pstat_LOP_act_buffer_h, Pstat_LSTP_act_buffer_h] = power_stat(obj)
            ...
            % HP
            Pstat_HP_act_buffer_h      = (8 * Pstat_HP_mux2to1) + Pstat_HP_decoder3to8 + (160 * Pstat_HP_mux8to1) + (8 * Pstat_HP_reg); % [W]
            % LOP
            Pstat_LOP_act_buffer_h      = (8 * Pstat_LOP_mux2to1) + Pstat_LOP_decoder3to8 + (160 * Pstat_LOP_mux8to1) + (8 * Pstat_LOP_reg); % [W]
            % LSTP
            Pstat_LSTP_act_buffer_h      = (8 * Pstat_LSTP_mux2to1) + Pstat_LSTP_decoder3to8 + (160 * Pstat_LSTP_mux8to1) + (8 * Pstat_LSTP_reg); % [W]
		end
    end
end
