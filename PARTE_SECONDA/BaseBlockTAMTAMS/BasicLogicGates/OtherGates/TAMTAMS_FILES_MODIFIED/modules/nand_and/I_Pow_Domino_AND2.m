%% DOMINO NAND 2 POWER ANALYSIS
% Implementation of a model for the evaluation of power consumption of 2 inputs DOMINO NAND-AND gates
%%
...
% Module name: I_Pow_Domino_AND2.m
% Stored informations:  Iand2_dyn_btb, Iand2_dyn_gate, Iand2_dyn_off, Pand2,  Pand2_dyn
...
% Width of pull-down nMOS transistor
Wn_clk=kclock.*fan_in.*Wgate;						% um
% Width of nMOS clock transistor 
Wp_clk=alpha_W.*Wn_clk;							% um
% Width of pMOS clock transistor
Wn_inv=Wgate; 								% um
% The width of the inveter nMOS
Wp_inv=alpha_W.*Wgate; 							% um 
% The width of the inveter pMOS 

% The number of the variable means the value of the inputs

% Ibtb of the inverter
Ibtb_n_inv=Ibtb_nmos.*Wn_inv;      	% nA
Ibtb_p_inv=Ibtb_pmos.*Wp_inv;      	% nA

% We consider the source of clock nMOS different from the drain of the the pull down transistor

Ibtb_clk_p_dyn2=Ibtb_pmos.*Wp_clk;      	% nA
Ibtb1_clk_n_dyn2=Ibtb_nmos_1.*Wn_clk;		% nA
Ibtb1_2_clk_n_dyn2=Ibtb_nmos_1_2.*Wn_clk;	% nA
Ibtb1_3_clk_n_dyn2=Ibtb_nmos_1_3.*Wn_clk;	% nA
Ibtb_n_dyn2=Ibtb_nmos.*Wn_PD;   		% nA
Ibtb1_n_dyn2=Ibtb_nmos_1.*Wn_PD;   		% nA
Ibtb1_2_n_dyn2=Ibtb_nmos_1_2.*Wn_PD;   		% nA
Ibtb1_3_n_dyn2=Ibtb_nmos_1_3.*Wn_PD;   		% nA
Ibtb2_3_n_dyn2=Ibtb_nmos_2_3.*Wn_PD;   		% nA

% CLOCK = 0 ----> CHARGE PHASE

I_0_0=Ibtb_n_dyn2+Ibtb2_3_n_dyn2+Ibtb1_3_n_dyn2+Ibtb1_3_clk_n_dyn2;	%nA
I_0_1=Ibtb_n_dyn2+2.*Ibtb1_2_n_dyn2+Ibtb1_2_clk_n_dyn2;			%nA
I_1_0=Ibtb_n_dyn2+Ibtb1_n_dyn2+Ibtb1_2_n_dyn2+Ibtb1_2_clk_n_dyn2;	%nA
I_1_1=Ibtb_n_dyn2+2.*Ibtb1_n_dyn2+Ibtb1_clk_n_dyn2;			%nA

Iinv=Ibtb_p_inv;	%nA
%In the all cases we have '1' at the input of the inverter

I0=((I_0_0+I_0_1+I_1_0+I_1_1)./4)+Iinv;	%nA
%current in the charge phase
  		
% CLOCK = 1 ----> EVALUATION PHASE

I_0_0=Ibtb_n_dyn2+Ibtb1_2_n_dyn2;	%nA
I_0_1=Ibtb_n_dyn2;			%nA
I_1_0=Ibtb_n_dyn2+Ibtb1_n_dyn2;		%nA
I_1_1=Ibtb_clk_p_dyn2;			%nA

Iinv=0.75.*Ibtb_p_inv+0.25.*Ibtb_n_inv;	%nA
%in three cases we have 1 as input of inverter and only one 0;

I1=((I_0_0+I_0_1+I_1_0+I_1_1)./4)+Iinv;	%nA
%current in evaluation phase

%we must divide per 2 because we have two phase clock.   

Iand2_dyn_btb(counter_mod_index, counter_tech_index, i)=(I0+I1)./2;	% nA

Ibtb_nand_and(counter_mod_index, counter_tech_index,i)=Iand2_dyn_btb(counter_mod_index, counter_tech_index,i);

MULTIVDD=[Vdd(counter_tech_index), VDD2]; % V 
% Power supply Vector

Pand2_btb(counter_mod_index,counter_tech_index,i)=0;

for j=1:length(MULTIVDD)
		Pand2_btb(counter_mod_index,counter_tech_index,i)=Pand2_btb(counter_mod_index,counter_tech_index,i)+Iand2_dyn_btb(counter_mod_index,counter_tech_index,i).*MULTIVDD(j).*1e-9; % W
...
Igate_clk_p_dyn2=Igate_pmos.*Wp_clk;	%nA
%Gate current due to the clock pMOS

I_0_0=Igate_clk_p_dyn2;		%nA
I_0_1=Igate_clk_p_dyn2;		%nA
I_1_0=Igate_clk_p_dyn2;		%nA
I_1_1=Igate_clk_p_dyn2;		%nA
...
Igate_clk_n_dyn2=Igate_nmos.*Wn_clk;		%nA
%Gate current due to the clock nMOS
Igate_n_dyn2=Igate_nmos.*Wn_PD;			%nA

I_0_0=Igate_clk_n_dyn2;				%nA
I_0_1=Igate_n_dyn2+Igate_clk_n_dyn2;		%nA
I_1_0=Igate_clk_n_dyn2;				%nA
I_1_1=2.*Igate_n_dyn2+Igate_clk_n_dyn2;		%nA

Iinv=0.75.*Igate_n_inv+0.25.*Igate_p_inv;	%nA
%in three cases we have 1 as input of inverter and only one 0;
...
Iand2_dyn_gate(counter_mod_index, counter_tech_index, i)=(I0+I1)./2;	% nA

Igate_nand_and(counter_mod_index, counter_tech_index,i)=Iand2_dyn_gate(counter_mod_index, counter_tech_index,i) ;

MULTIVDD=[Vdd(counter_tech_index), VDD2]; % V 
% Power supply Vector

Pand2_gate(counter_mod_index,counter_tech_index,i)=0;

for j=1:length(MULTIVDD)
		Pand2_gate(counter_mod_index,counter_tech_index,i)=Pand2_gate(counter_mod_index,counter_tech_index,i)+Iand2_dyn_gate(counter_mod_index,counter_tech_index,i).*MULTIVDD(j).*1e-9; % W
...
%Ioff and Igate of the inverter
Ioff_n_inv=Ioff_nmos.*Wn_inv;      	% nA
Ioff_p_inv=Ioff_pmos.*Wp_inv;      	% nA

% CLOCK = 0 ----> CHARGE PHASE
% Now we are considering the Ioff current due to the nMOS clock transistor only because it is bigger than the ones of pull down network. There are three different Ioff contributes because we have three transistors in series.

Ioff_clk_n_dyn2=Ioff_nmos_1.*Wn_clk;	%nA
Ioff2_clk_n_dyn2=Ioff_nmos_2.*Wn_clk;	%nA
Ioff3_clk_n_dyn2=Ioff_nmos_3.*Wn_clk;	%nA

I_0_0=Ioff3_clk_n_dyn2;		%nA
I_0_1=Ioff2_clk_n_dyn2;		%nA
I_1_0=Ioff2_clk_n_dyn2;		%nA
I_1_1=Ioff_clk_n_dyn2; 		%nA
...
Ioff_clk_p_dyn2=Ioff_pmos.*Wp_clk;		%nA
%Subthreshold current due to the clock pMOS
Ioff_n_dyn2=Ioff_nmos_1.*Wn_PD;			%nA
Ioff2_n_dyn2=Ioff_nmos_2.*Wn_PD;			%nA

I_0_0=Ioff2_n_dyn2;		%nA
I_0_1=Ioff_n_dyn2;		%nA
I_1_0=Ioff_n_dyn2;		%nA
I_1_1=Ioff_clk_p_dyn2;		%nA

Iinv=0.75.*Ioff_p_inv+0.25.*Ioff_n_inv;	%nA
%in three cases we have 1 as input of inverter and only one 0;
...
Iand2_dyn_off(counter_mod_index, counter_tech_index, i)=(I0+I1)./2;	% nA

Ioff_nand_and(counter_mod_index, counter_tech_index,i)=Iand2_dyn_off(counter_mod_index, counter_tech_index,i);

MULTIVDD=[Vdd(counter_tech_index), VDD2]; % V 
% Power supply Vector

Pand2_off(counter_mod_index,counter_tech_index,i)=0;

for j=1:length(MULTIVDD)
		Pand2_off(counter_mod_index,counter_tech_index,i)=Pand2_off(counter_mod_index,counter_tech_index,i)+Iand2_dyn_off(counter_mod_index,counter_tech_index,i).*MULTIVDD(j).*1e-9; % W 

endfor

endfor

% POWER STATIC Domino AND2 

% Vdd_d is the distribution of Vdd in multi-supply voltage chip
% Vdd_d and VDD2 ---> datibase.m

Pand2(counter_mod_index,counter_tech_index,:)=Pand2_off(counter_mod_index,counter_tech_index,:)+Pand2_gate(counter_mod_index,counter_tech_index,:)+Pand2_btb(counter_mod_index,counter_tech_index,:); % W

Pnand_and(counter_mod_index,counter_tech_index,:)=Pand2(counter_mod_index,counter_tech_index,:);

% POWER DYNAMIC Domino AND2 

for i=1:4

	Pand2_dyn(counter_mod_index,counter_tech_index,i)=0;
	for j=1:length(MULTIVDD)
			Pand2_dyn(counter_mod_index,counter_tech_index,i)=Pand2_dyn(counter_mod_index,counter_tech_index,i)+(Cand2_dyn(counter_mod_index,counter_tech_index,i).*1e-12).*Vdd_d(j).*MULTIVDD(j).^2.*freq(counter_mod_index,counter_tech_index).*1e6; % W 
	endfor

endfor

Pnand_and_dyn(counter_mod_index,counter_tech_index,:)=Pand2_dyn(counter_mod_index,counter_tech_index,:);
